...
- 예약, 결제, 예약 취소, 내 글에 댓글이 달렸을 때, 문의글의 답변이 달렸을 때, 포인트 사용, 적립 총 7가지 상황에 알림이 옴
- 짧은 알림음과 함께 모든 페이지 오른쪽 하단에서 올라왔다가 수 초 후 내려가 사라지는 애니메이션 적용
- 알림은 읽지 않은 알림의 총 갯수를 보여주며 클릭 시 알림 페이지로 이동하며 해당 페이지 진입 시 모든 알림은 읽음 처리 되어 새로운 알림 상황이 오지 않을 시 알림이 울리지 않음.
- 알림 페이지에서 알림 내역 클릭 시 각 관련 페이지들로 

**8. 채팅**<br>
<left><img src="chat.gif" /></left><br>
- Websocket을 이용하여 사용자-사업자의 실시간 채팅 가능
- 모든 대화는 저장되어 이전의 대화도 볼 수 있음
- 채팅목록, 예약 상세 페이지, 업체 상세 페이지에서 채팅 바로 연결 가능
