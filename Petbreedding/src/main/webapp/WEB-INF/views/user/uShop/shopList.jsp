                    ...
                    <div class="Store">                      
                       <li class="storeList">
                            ...
                            <div class="storeList_inner">
                                ...
                                <div class="storeList_etc_area">
                                    <a href="#">리뷰<small> 
                                    <c:if test="${loc_new_revAvg[status.index] ge 0 && loc_new_revAvg[status.index] lt 1}">
                                            &nbsp;(${loc_new_revAvg[status.index] })
                                       </c:if>
                                    <c:if test="${loc_new_revAvg[status.index] ge 1 && loc_new_revAvg[status.index] lt 2}">
                                            <i class="fas fa-star rate"></i>&nbsp;(${loc_new_revAvg[status.index] })
                                       </c:if>
                                       <c:if test="${loc_new_revAvg[status.index] ge 2 && loc_new_revAvg[status.index] lt 3}">
                                            <i class="fas fa-star rate"></i>
                                            <i class="fas fa-star rate"></i>&nbsp;(${loc_new_revAvg[status.index] })
                                       </c:if>
                                       <c:if test="${loc_new_revAvg[status.index] ge 3 && loc_new_revAvg[status.index] lt 4}">
                                            <i class="fas fa-star rate"></i>
                                            <i class="fas fa-star rate"></i>
                                            <i class="fas fa-star rate"></i>&nbsp;(${new_revAvg[status.index] })
                                       </c:if>
                                       <c:if test="${loc_new_revAvg[status.index] ge 4 && loc_new_revAvg[status.index] lt 5}">
                                            <i class="fas fa-star rate"></i>
                                            <i class="fas fa-star rate"></i>
                                            <i class="fas fa-star rate"></i>
                                            <i class="fas fa-star rate"></i>&nbsp;(${loc_new_revAvg[status.index]})
				            ...
				            <div class="page_wrap">
				               <div class="page_nation">
				                  <c:if test="${loc_new_paging.startPage != 1 }">
				                     <a class="arrow prev" href="${path}/shopList/all?shopType=${shopType}&nowPage=${loc_new_paging.startPage - 1 }&cntPerPage=${loc_new_paging.cntPerPage}">이전</a> 
				                  </c:if>
				                  <c:forEach begin="${loc_new_paging.startPage }" end="${loc_new_paging.endPage }" var="p">
				                     <c:choose>
				                        ...
				                        <c:when test="${p != loc_new_paging.nowPage }">
				                           <a href="${path}/shopList/all?shopType=${shopType}&nowPage=${p }&cntPerPage=${loc_new_paging.cntPerPage}">${p}</a>
				                        </c:when>
				                     </c:choose>
				                  </c:forEach>
				                  <c:if test="${loc_new_paging.endPage != loc_new_paging.lastPage}">
				                     <a class="arrow next" href="${path}/shopList/all?shopType=${shopType}&${loc_new_paging.endPage+1 }&cntPerPage=${loc_new_paging.cntPerPage}">다음</a>
				                  </c:if>
				               </div>
				            </div>
          			  <!-- 페이징 끝! -->
